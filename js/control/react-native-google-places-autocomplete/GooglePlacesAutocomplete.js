...
const GooglePlacesAutocomplete = React.createClass({
  ...
  propTypes: {
    ...
    isFocusText: React.PropTypes.func,
  ...
  getCurrentLocation() {
    this._requestNearby(this.props.latitude, this.props.longitude);
  },
  ...
  _onPress(rowData) {
    if (rowData.isPredefinedPlace !== true && this.props.fetchDetails === true) {
      ...
      this._enableRowLoader(rowData);
      if(rowData.description != this.props.currentLocationLabel)
        this.setState({
          text: rowData.description,
        });
      else 
    ...
    if (latitude !== undefined && longitude !== undefined && latitude !== null && longitude !== null) {
      ...
      if (this.props.nearbyPlacesAPI === 'GoogleReverseGeocoding') {
        ...
        url = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?' + Qs.stringify({
          ...
          key: this.props.query.key,
          radius: 1000,
          types: ['airport', 'amusement_park', 'aquarium', 'art_gallery', 'bakery', 'bar', 'bowling_alley', 'bus_station', 'cafe', 'campground', 'casino', 'city_hall', 'convenience_store', 'courthouse', 'embassy', 'department_store', 'gas_station', 'gym', 'home_goods_store', 'hospital', 'library', 'liquor_store', 'local_government_office', 'meal_delivery', 'meal_takeaway', 'movie_theater', 'museum', 'night_club', 'park', 'restaurant', 'rv_park', 'shopping_mall', 'spa', 'stadium', 'subway_station', 'train_station', 'transit_station', 'university', 'zoo', 'lodging'],
  ...
  _request(text) {
    ...
    if (text.length >= this.props.minLength) {
      ...
      // request.open('GET', 'https://maps.googleapis.com/maps/api/place/textsearch/json?query=' + encodeURIComponent(text) + '&key=' + this.props.query.key);      
      request.send();
    } else {
      this._results = [];
      this.setState({
      ...
      >
        <View>
          <View style={[defaultStyles.row, this.props.styles.row, rowData.isPredefinedPlace ? this.props.styles.specialItemRow : {}]}>
            <Icon name='ios-pin' style={{fontSize:20,color:'#74cdc1'}}/>
            <Text
  ...
  _onBlur() {
    this.triggerBlur();
    this.props.isFocusText(false);
    this.setState({listViewDisplayed: false});
  },

  _onFocus() {
    this.props.isFocusText(true);
