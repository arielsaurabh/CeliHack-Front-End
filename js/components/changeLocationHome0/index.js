...
let Card = React.createClass({
  render() {
    ...
    if (Global.current_lat === 0) {
        ...
        distance = ''+ Math.round(Util.getDistanceFromLatLonInKm(Global.changed_lat,Global.changed_lng,restaurantLat,restaurantLng))+' km away';
    }
    return (
...
class ChangeLocationHome0 extends Component {
    constructor(props) {
        super(props);
        _this = this;
        this.state = {
    ...
    loadDishData() {
        ...
        if (Global.isGuest === true) {
            api = 'dish/all_guest';
            formdata.append('Latitude', Global.changed_lat);
            formdata.append('Longitude', Global.changed_lng);
        } else {
            api = 'dish/all_user';
                ...
                <Header style={{ backgroundColor:theme.brandPrimary, justifyContent: 'space-between', height: theme.headerHeight}}>
                    <Grid>
                        ...
                        <Col style={[styles._alignCol, {flex:8}]}>
                            <Text numberOfLines={1} style={styles._titleText}>{this.props.locationName}</Text>
                        </Col>
                        <Col style={{flex:1}}>
                        </Col>
                    </Grid>
...
export default connect(null, bindAction)(ChangeLocationHome0);
