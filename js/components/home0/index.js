...
import { Platform, Modal, View, Text, TouchableOpacity, TouchableHighlight, Image, Dimensions, Alert ,AsyncStorage, Linking } from 'react-native';
                ...
                <View>
                    <View style={styles._cardDishNameView}>
                        <View style={{flex:1, flexDirection:'row', justifyContent:'flex-end'}}>
                            <Text numberOfLines={1} style={{flex:3,color:'#74cdc1',textDecorationLine:'underline', marginLeft: 5}}>{distance}</Text>
                            <Text numberOfLines={1} style={{flex:7, textAlign:'right', marginRight:5, }}>{this.props.DishName}</Text>
                        </View>
...
class Home0 extends Component {
    ...
    getCurrentCoordinate() {
        if ( Platform.OS == 'ios' && Global.current_lat == 0 ) {
            ...
            LocationServicesDialogBox.checkLocationServicesIsEnabled({
                message:"<h6>רוצה לראות המלצות על מנות ללא גלוטן שהעלו צליאקים בסביבה שלך? הפעל את ה- GPS! </h6>לא תוכל להשתמש באפליקציה בלי GPS  פעיל. הרי לא נוכל לראות לך מנות ללא גלוטן באזור שלך אם לא נדע איפה אתה!", 
                ok: "כן",
                cancel: "לא"
    ...
    loadCategoriesData() {
        ...
        .then((responseData) => {
            ...
            for(var i = 1; i < Global.categoryList.length ; i ++ ) {
                Global.categoriesHb.push(Global.categoryList[i].Category_H);
            }
        })
        .done();        
    }
    ...
    gotoSearch() {
        Alert.alert(
            ...
            'אפשרויות חיפוש:',
            [
                {text: 'שנה מיקום',     onPress: () => alert('change location')},
                {text: 'חפש לפי מסעדה', onPress: () => alert('search restaurant')},
            ]
        );
    }
                ...
                <Header style={{ backgroundColor:theme.brandPrimary, justifyContent: 'space-between', height: theme.headerHeight}}>
                    <Grid>
                        <Col style={[styles._alignCol, {flex:5}]}>
                            <Text style={styles._titleText}>CeliHack</Text>
                        </Col>
                        <Col style={[styles._alignCol, {flex:5}]}>
                            <TouchableOpacity style={styles._alignCol}
                                ...
                                <Text style={{color:'#FFF'}}>חיפוש</Text>
                            </TouchableOpacity>
                        </Col>
                        <Col style={[styles._alignCol, {flex:5}]}>
                            <TouchableOpacity style={styles._alignCol}
                                                ...
                                                onPress={() => 
                                                    {
                                                        ...
                                                        this.navigateTo('changeLocation0');
                                                    }}>
                                            ...
                                            onPress={() => 
                                                {
                                                    ...
                                                    this.setModalVisible(!this.state.modalVisible);
                                                    this.navigateTo('searchRestaurant0');
                                                }}>
                                            <Text style={{ color:'grey', fontSize:15}}>
                                                 חפש לפי מסעדה
                                            </Text>
                    ...
                    <View style={styles._footer}>
                        ...
                        <View style={[styles._alignCol, {flex:10}]}>
                            <TouchableOpacity style={[styles._alignCol, {flexDirection:'row'}]} onPress={() => this.gotoCategories()}>
                                <Text style={styles._allFoodCategory}>כל קטגוריות האוכל</Text>
...
export default connect(null, bindAction)(Home0);
